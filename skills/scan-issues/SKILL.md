...
### 5. PR Auto-Merge Constraint

Only **one PR can be open at a time**. The `create-feature` → `commit-push` pipeline within `fix-issue` handles PR creation. After a PR is opened:

- Set it to auto-merge if the repo supports it:
  ```bash
  gh pr merge "$PR_NUMBER" --auto --merge --repo avoidwork/madz
  ```
